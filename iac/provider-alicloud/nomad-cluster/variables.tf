...
variable "client_server_nested_virtualization" {
  type        = bool
  description = "AlibabaCloud nested virt requires bare-metal (ebmg/ebmgn). Standard g7 instance families do NOT support KVM nested."
}
...
variable "alicloud_acr_instance_id" {
  type        = string
  description = "ACR EE instance id (cri-xxx). Passed through to nodepool so user_data writes to /etc/aliyun-acr-helper.conf."
}
